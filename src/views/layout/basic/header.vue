...
.header-content(ref="headerRef")
  header(
    class="fixed top-0 inset-x-0 z-30 h-12 transition duration-500"
    :class="{'bg-white/90': windowY > 0}"
  )
    div(class="container h-full flex justify-between items-center")
      ...
      div(class="flex items-center")
        ...
        LanguageComp(class="ml-3 md:ml-6 w-5 h-5 text-sm")
        button(class="ml-3 md:ml-6 w-5 h-5 md:hidden" @click="isMenu = !isMenu")
          Bars3Icon(v-if="!isMenu")
          XMarkIcon(v-else)
  TransitionRoot(:show="isMenu")
    .mobile-memu(class="fixed inset-0 z-20 pt-12 bg-orange-50/95 md:hidden")
      div(v-for="(menu, index) in MENU_LIST" :key="menu.title")
        TransitionChild(
          ...
          router-link(
            :to="{ name: menu.name, hash: menu.hash }"
            class="block text-center py-3 border-t text-xs sm:text-sm"
            @click="isMenu = false"
          ) {{ $t(menu.title) }}
</template>
...
import LanguageComp from '@/components/language-dropdown/index.vue'
